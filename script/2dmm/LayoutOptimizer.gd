## 力导向布局优化器，使用斥力-引力模型自动排列思维导图节点，检测和解决节点重叠
extends RefCounted
...
## 初始化布局优化器，绑定数据管理器和思维导图
func _init(dm: RefCounted, mm_2d: RefCounted) -> void:
	data_manager = dm
	mindmap_2d = mm_2d

## 执行力导向布局优化，迭代计算斥力、引力和中心重力直到收敛
