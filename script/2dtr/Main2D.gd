## 2D认知导图独立主界面，提供完整的节点管理、布局、快捷键和截图功能
extends Control
...
func _ready() -> void:
	...
	var CognitiveMap2DScript := load("res://script/2dtr/CognitiveMap2D.gd")
	if not CognitiveMap2DScript:
		_log("无法加载认知地图脚本")
		return

	mindmap_2d = CognitiveMap2DScript.new(data_manager, self)
	if _debug_logger:
